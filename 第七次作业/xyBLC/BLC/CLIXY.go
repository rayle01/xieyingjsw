...
func (cli *CLIXY) RunXY() {
	...
	resetMethodCmd:=flag.NewFlagSet("reset",flag.ExitOnError)
	...
	flagMinerData:=startNodeCmd.String("miner","","定义挖矿奖励的地址")
	...
	case "reset":
		err := resetMethodCmd.Parse(os.Args[2:])
		if err != nil {
			log.Panic(err)
		}
	case "startnode":
	...
	if sendCmd.Parsed() {
		...
		//fmt.Println(to)
		//fmt.Println(amount)
		cli.SendXY(from, to, amount,nodeID)
	}

	if printChainCmd.Parsed() {
		//fmt.Println("打印区块。。。")
		//cli.BlockChain.PrintChains()
		cli.PrintChainsXY(nodeID)
	}
	...
	if CreateBlockChainCmd.Parsed() {
		...
		cli.CreateBlockChainXY(*flagCreateBlockChainData,nodeID)
	}
	...
	if getBalanceCmd.Parsed() {
		...
		cli.GetBalanceXY(*flagGetBalanceData,nodeID)
	}

	if createWalletCmd.Parsed() {
		//创建钱包--->交易地址
		cli.CreateWalletXY(nodeID)
	}


	if getAddresslistsCmd.Parsed(){
		cli.GetAddressListsXY(nodeID)
	}


	if resetMethodCmd.Parsed(){
		cli.ResetMethodXY(nodeID)
	}
	if startNodeCmd.Parsed(){
		cli.StartNodeXY(nodeID,*flagMinerData)
	}
...
func printUsageXY() {
	...
	fmt.Println("\tstartnode -miner Address -- 启动节点")
}

...
//	return GetAddressByPublicKey(w.PublickKey)
	return GetAddressByPublicKeyXY(w.PublickKey)
}

func GetAddressByPublicKeyXY(publicKey []byte)[]byte{
	pubKeyHash:=PubKeyHashXY(publicKey)
	return GetAddressByPubKeyHashXY(pubKeyHash)
}

func GetAddressByPubKeyHashXY(pubKeyHash []byte) []byte {
	//step2：添加版本号：

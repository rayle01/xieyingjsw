...
func CreateBlockChainWithGenesisBlockXY(address string) {
	...
	genesisBlock := CreateGenesisBlockXY([]*TransactionXY{txCoinBase})
...
func (bc *BlockChainXY) AddBlockToBlockChainXY(txs []*TransactionXY) {
	...
	err := bc.DB.Update(func(tx *bolt.Tx) error {
		...
		if b != nil {
			...
			//创建新的区块
			newBlock := NewBlockXY(txs, lastBlock.Hash, lastBlock.Height+1)
			//序列化后存入到数据库中
 ...
 */
func (bc *BlockChainXY) UnSpentXY(address string, txs [] *TransactionXY) []*UTXOXY { //王二狗
	/*
	 ...
	 */
	//存储未花费的TxOutput
	var unSpentUTXOs [] *UTXOXY
	//存储已经花费的信息
	spentTxOutputMap := make(map[string][]int) // map[TxID] = []int{vout}

	//第一部分：先查询本次转账，已经产生了的Transanction
	for i := len(txs)-1;i>=0;i--{
		unSpentUTXOs = caculateXY(txs[i],address,spentTxOutputMap,unSpentUTXOs)
	}
	...
	for {
		...
		for i := len(block.Txs) - 1; i >= 0; i-- {
			unSpentUTXOs = caculateXY(block.Txs[i],address,spentTxOutputMap,unSpentUTXOs)
		}
...
func caculateXY(tx *TransactionXY,address string, spentTxOutputMap map[string][]int,unSpentUTXOs []*UTXOXY) []*UTXOXY{
	//遍历每个tx：txID，Vins，Vouts

	//遍历所有的TxInput
	if !tx.IsCoinBaseTransactionXY() { //tx不是CoinBase交易，遍历TxInput
		...
		if txOutput.UnlockWithAddress(address) {
			if len(spentTxOutputMap) != 0 {
				...
				if !isSpentOutput {
					...
					utxo := &UTXOXY{tx.TxID, index, txOutput}
...
*/
func (bc *BlockChainXY) FindSpentableUTXOsXY(from string, amount int64,txs[]*TransactionXY) (int64, map[string][]int) {
	/*
	1.根据from获取到的所有的utxo
...
//提供一个功能：查询余额
func (bc *BlockChainXY) GetBalanceXY(address string,txs[] *TransactionXY) int64 {
	//txOutputs := bc.UnSpent(address)
	unSpentUTXOs := bc.UnSpentXY(address,txs)
...
func (bc *BlockChainXY) MineNewBlockXY(from, to, amount []string) {
	...
	for i := 0; i < len(from); i++ {
		...
		tx := NewSimpleTransactionXY(from[i], to[i], amountInt, utxoSet,txs)
		txs = append(txs, tx)
...
//	创建一个CoinBase交易--->Tx
	...
	//2.新建区块
	newBlock := new(BlockXY)
	err := bc.DB.View(func(tx *bolt.Tx) error {
		b := tx.Bucket([]byte(BlockBucketName))
		if b != nil {
			//读取数据库
			blockBytes := b.Get(bc.Tip)
			lastBlock := DeserializeBlockXY(blockBytes)
	...
	err = bc.DB.Update(func(tx *bolt.Tx) error {
		...
		if b != nil {
			//将新block存入到数据库中
			b.Put(newBlock.Hash, newBlock.SerializeXY())
			//更新l
			b.Put([]byte("l"), newBlock.Hash)
			//tip
			bc.Tip = newBlock.Hash
